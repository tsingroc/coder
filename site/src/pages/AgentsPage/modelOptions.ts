...
export const getModelSelectorPlaceholder = (
	...
	if (modelOptions.length > 0) {
		return t("model.selector.selectModel");
	}
	if (isModelCatalogLoading) {
		return t("model.selector.loadingModels");
	}
	if (hasConfiguredModels) {
		return t("model.selector.noAvailableModels");
	}
	return t("model.selector.noModelsConfigured");
};
...
export const getModelCatalogStatusMessage = (
	...
	if (isModelCatalogLoading) {
		return t("model.status.loadingModelCatalog");
	}
	if (hasModelCatalogError) {
		return t("model.status.modelCatalogUnavailable");
	}
	if (hasConfiguredModelsInCatalog(catalog)) {
		return t("model.status.modelsConfiguredUnavailable");
	}
	return t("model.status.noChatModelsConfigured");
};
