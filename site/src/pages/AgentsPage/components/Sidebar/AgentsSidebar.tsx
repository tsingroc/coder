...
const getModelDisplayName = (
	...
	if (!provider || !model) {
		return modelConfig.display_name.trim() || t("common.defaultModel");
	}
...
const ChatTreeNode = memo<ChatTreeNodeProps>(({ chat, isChildNode }) => {
	...
	const filesChangedLabel = `${changedFiles} ${
		changedFiles === 1 ? t("common.file_one") : t("common.file_other")
	}`;
							...
							<DropdownMenu>
								<DropdownMenuTrigger asChild>
									<Button
										...
										aria-label={t("tree.openActionsFor", { title: chat.title })}
									>
										...
										<>
											...
											{workspaceId && (
												...
												>
													...
													{t("menu.archiveAndDeleteWorkspace")}
												</DropdownMenuItem>
											)}
										</>
									)}
						...
						<DropdownMenu>
							<DropdownMenuTrigger asChild>
								<Button
									variant="subtle"
									size="icon"
									aria-label={t("sidebar.filterAgents")}
									className={cn(
